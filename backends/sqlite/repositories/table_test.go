...
func TestSQLiteTableNameValidation(t *testing.T) {
	...
	quoted, err = repositories.ValidateAndQuoteTableName("main.jobs")
	require.NoError(t, err)
	require.Equal(t, "\"main\".\"jobs\"", quoted)

	quoted, err = repositories.ValidateAndQuoteTableName("main.select")
	require.NoError(t, err)
	require.Equal(t, "\"main\".\"select\"", quoted)
